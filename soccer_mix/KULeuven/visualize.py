...
def show_location_submodel(loc_submodel, show=True, figsize=6):
    ax = mps.field(show=False, figsize=figsize)
    add_ellips(ax, loc_submodel.mean, loc_submodel.cov)
    if show:
        plt.show()
...
def show_component_differences(loc_models, dir_models, vec_p1, vec_p2, name1, name2, save=True):
    actions_to_save = {'dribble': 'Drible', 'pass': 'Passe', 'shot': 'Chute', 'pressure': 'Pressão', 'receival': 'Recepção de Passe'}
    
    # determine colors of dir sub models
    difference = vec_p2 - vec_p1
    ...
    for loc_model in loc_models:
        ...
        plt.axis("scaled")
        if loc_model.name in actions_to_save:
            axs.append(ax)
            if save:
                plt.title(name1 + ' x ' + name2 + ' - ' + actions_to_save[loc_model.name], fontdict={'fontsize': 24})
                savefigure(f"E:\Galo\Data\VectorViz\{name1}-{name2}-{actions_to_save[loc_model.name]}.png")
            else:
                plt.title(actions_to_save[loc_model.name], fontdict={'fontsize': 24})

    return axs

def show_component_proportions(loc_models, dir_models, vec_p1, vec_p2, name1, name2, save=True):
    # determine colors of dir sub models
...
def show_vector_proportions(loc_models, dir_models, vec_p1, name1, save=True):
    # determine colors of dir sub models
    ...
    for loc_model in loc_models:
        ...
        maxi = max(vec_p1.loc[vec_p1.index.str.contains(f"^{loc_model.name}_")])
        ...
        for i, gauss in enumerate(loc_model.submodels):
            ...
            if (am_subclusters == 1).all():
                add_ellips(ax, gauss.mean, gauss.cov,
                           color=cmap(norm(vec_p1.loc[f"{loc_model.name}_{i}_0"])), alpha=1)

            elif f"{loc_model.name}_{i}" != 'shot_4':
                add_ellips(ax, gauss.mean, gauss.cov, color='gainsboro')
                ...
                for dir_model in dir_models:
                    if f"{loc_model.name}_{i}" == dir_model.name:
                        ...
                        for j, vonmises in enumerate(dir_model.submodels):
                            ...
                            dy = np.sin(vonmises.loc)[0]
                            add_arrow(ax, x, y, 10 * dx, 10 * dy,
                                      fc=cmap(norm(vec_p1.loc[f"{loc_model.name}_{i}_{j}"])),
                                      arrowsize=4.5, linewidth=1
                                      )
        ...
        if loc_model.name in actions_to_save:
            axs.append(ax)
            if save:
                plt.title(name1 + ' - ' + actions_to_save[loc_model.name], fontdict={'fontsize': 24})
                savefigure(f"E:\Galo\Data\VectorViz\{name1} - {actions_to_save[loc_model.name]}.png")
            else:
                plt.title(actions_to_save[loc_model.name], fontdict={'fontsize': 24})
